...
`TECHNICAL_CANDIDATE_STATUS = PASS` signifie que les gates techniques automatisées et les preuves de candidate sont passées ; il ne signifie pas qu’une release finale est publiée. `TECHNICAL_STATUS_LEGACY_ALIAS` peut être conservé uniquement pour compatibilité de lecture et ne constitue pas le label canonique. `RELEASE_STATUS = CANDIDATE` reste obligatoire tant qu’un tag immuable, un asset publié et leur comparaison finale ne sont pas approuvés. `FINAL_RELEASE = PENDING_NOT_PUBLISHED` reste obligatoire avant publication. `UX_CONTENT = PASS` exige la revue UX/WCAG et les attestations natives. `COMMERCIAL_RELEASE = PASS` exige la décision produit/client. `ULTRA_PREMIUM = PASS` exige les trois décisions, l’enveloppe de charge complète, la compatibilité d’upgrade, les opérations backup/restore/rollback et l’absence de réserve bloquante.
