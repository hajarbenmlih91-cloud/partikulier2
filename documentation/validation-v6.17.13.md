# Validation de la candidate v6.17.13

La candidate v6.17.13 est dérivée de `v6.17.12@6a66c5ff0db8341cd5797cd5cf1b73e5573dc3ce` et comparée à la base CDC `v6.17.7@6153debac8f84da46b1da95af1c810320dc7e5bf`. Les tags v6.17.7 et v6.17.11 ne sont pas réécrits.
...
La recette locale a été orchestrée par `scripts/ci-cold-acceptance.sh` sur un répertoire WordPress et une base neufs, avec mot de passe SQL contenant une apostrophe. Le résultat observé est `COLD_ACCEPTANCE=PASS` sur `http://localhost:8098`.
...
Le workflow `.github/workflows/cdc-v6.17.13.yml` impose l’ordre `static-contracts → cold-acceptance → package → release`. Le job `cold-acceptance` reconstruit l’environnement et publie ses preuves. Le job `package` dépend de l’acceptance et vérifie le bundle. Le job `release` dépend du package, est limité au tag `v6.17.13`, vérifie le checksum du ZIP et ne crée la release qu’après succès des étapes précédentes.
