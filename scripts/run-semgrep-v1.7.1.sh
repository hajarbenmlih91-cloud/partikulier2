...
OUT="${1:-$ROOT/documentation/semgrep-v1.7.1.json}"
...
cat "$RAW" | jq --arg commit "$COMMIT" --arg command 'semgrep --config .semgrep/partikulier.yml --json --output documentation/semgrep-v1.7.1.json <66 explicit theme/*.php targets>' --argjson rules "$rules_json" --argjson targets "$paths_json" --argjson bytes "$bytes" \
