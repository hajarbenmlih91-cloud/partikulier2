...
OUT="${1:-$ROOT/documentation/semgrep-v6.17.10.json}"
RAW="$(mktemp)"
trap 'rm -f "$RAW"' EXIT
cd "$ROOT"
...
cat "$RAW" | jq --arg commit "$(git rev-parse HEAD 2>/dev/null || echo uncommitted)" --arg command 'semgrep --config .semgrep/partikulier.yml --json --output documentation/semgrep-v6.17.10.json theme' --argjson rules "$rules_json" --argjson targets "$paths_json" --argjson bytes "$bytes" \
